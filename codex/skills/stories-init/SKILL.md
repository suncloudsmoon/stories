...
5. **Map the systems.** Choose the repo's real segments (block granularity). Author `docs/stories/systems/<segment>.md` for each (kind `system`, `covers:`, plain lead — per the systems-layer rules in the `stories` skill) and derive the root `ARCHITECTURE.md`: plain summary, top-level Mermaid flowchart, legend table linking every block to its page, `## New since <today>`, derived-by marker. Everything accounted for; a tiny repo may take the root file alone. Never touch a hand-written ARCHITECTURE.md (no marker) — surface it.
6. **Wire it up.** Add `origin.md` to the Atlas under `## Origin`, and each systems page under `## Systems`, with one-line hooks — for every gate-bearing page, list its `covers:` globs inline beside the hook; append a log entry; if a canon linter is available (`scripts/lint-canon.py`), run it and fix what it flags; report the tree.
