...
1. **Take in the source.** From the user's message: a file path → read it; a URL → fetch it; a topic with no source attached → say so and offer to run research; pasted content → use it directly.
...
5. **Atlas & log.** Add the page to `index.md` under `## Library`; append `## [<today>] ingest | <title>` to `log.md`; report what landed and what it linked to.
