...
## 6. Wire it up
Add `origin.md` to the Atlas under `## Origin`, and each systems page under `## Systems`, with one-line hooks — and for every gate-bearing page, list its `covers:` globs inline beside the hook. Append a log entry. If a canon linter is available (`scripts/lint-canon.py`), run it and fix what it flags. Report the tree you created.
