...
## [2026-07-05] feat | the loop closes (v1.1.0)
Six audit scouts returned one verdict: the discipline trusted and never verified. Now it verifies. The Atlas carries every gate-bearing page's `covers:` inline, so the gate resolves in one read; the contract's own homes joined the gate ([[the-gate]] covers `README.md` + the design spec, [[claude-code-home]] covers `CLAUDE.md` + `CHANGELOG.md`, [[canon-tooling]] covers CI); the after-change sweep and all three write-path commands close by running lint; and lint learned the checks the prose used to beg for — default-exempt sync across the three contract homes (an error), a required `path:line` on gate-bearing pages, a git-tracked coverage denylist, staleness that names the file that moved, command-drift that names the differing step. Drift repairs riding along: the `marketplace.json` removal (`f38da00`, 2026-07-04 — unlogged at the time, recorded here) leaves [[claude-code-home]]; the Codex ingest skill recovers the word "deep"; `AGENTS.md` learns the three-places rule; the README states the full exempt list. Manifests → 1.1.0.

## [2026-07-05] ship | v1.1.0 published
Tagged `v1.1.0` (`92a45bb`) and published the GitHub Release — the loop-closing feature set, live. CI green on the release commit (the first green since the 2026-07-04 workflow break the release itself fixed). Release notes carry the corrected install path: `claude plugin marketplace add suncloudsmoon/plugins` — the v1.0.0 release body still names the old in-repo marketplace, left as history.
